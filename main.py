...
def merge_opt_and_config(opt, config):
    ...
    if opt.pretrained_model:
        config.model.pretrained_model = os.path.join(proj_dir, opt.pretrained_model)
    else:
...
class ImageLogger(Callback):
    def __init__(self, batch_frequency, max_images, clamp=True, increase_log_steps=False,
        ...
        if not increase_log_steps:
            self.log_steps = [1, int(self.batch_freq) // 4, int(self.batch_freq) // 2]
        self.clamp = clamp
...
if __name__ == "__main__":
    ...
    if not opt.resume:
        if opt.train_from_scratch:
            ckpt_file=torch.load(config.model.pretrained_model,map_location='cpu')['state_dict']
            ckpt_file={key:value for key,value in ckpt_file.items() if not ( key[:6]=='model.')}
            model.load_state_dict(ckpt_file,strict=False)
            print("Train from scratch!")
        else:
            model.load_state_dict(torch.load(config.model.pretrained_model,map_location='cpu')['state_dict'],strict=False)
